...
class COMMONBASECODE_API UaVariant;
class COMMONBASECODE_API UaDataValue;

/** @ingroup CppBaseLibraryClass
 @brief Wrapper class for the UA stack structure T_DataTime.
 ...
 since January 1, 1601 (UTC). It is normally represented by the structure T_DataTime with the two UInt32
 ...
 - A DateTime value is the maximum int64_t value if either the value is equal to or greater than 9999-01-01 11:59:59PM
...
*/
class COMMONBASECODE_API UaDateTime
{
public:
    UaDateTime();
    UaDateTime(const T_DataTime&);
    UaDateTime(const Int64 &timedata);
    virtual ~UaDateTime();
    ...
    UaDateTime(const UInt64&);
    UaDateTime& operator=(const UInt64&);
    // operator uint64_t() const;

    operator T_DataTime() const;
    ...
    static UaDateTime fromString(const std::string&);
    ...
    int daysTo(const UaDateTime&) const;
    int secsTo(const UaDateTime&) const;
    int msecsTo(const UaDateTime&) const;
    void addSecs(int secs);
    void addMilliSecs(int msecs);

    void copyTo(T_DataTime *copy) const;

    void toDataValue(UaDataValue &dataValue, bool updateTimeStamps) const;
    void toDataValue(T_DataValue &dataValue, bool updateTimeStamps) const;

private:
    T_DataTime m_dateTime;

    // friend COMMONBASECODE_API UaDataStream &operator<<(UaDataStream &, const UaDateTime &);
    // friend COMMONBASECODE_API UaDataStream &operator>>(UaDataStream &, UaDateTime &);
};

// define UaDataStream operators for UaDateTime
// COMMONBASECODE_API UaDataStream &operator<<(UaDataStream &, const UaDateTime &);
// COMMONBASECODE_API UaDataStream &operator>>(UaDataStream &, UaDateTime &);
};
